...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        Environment environment = new Environment("dev",
                                                    new JdbcTransactionFactory(),       // 수동 커밋(자동 커밋은 ManagedTransactionFactory)
        ...
        /* 필기.
            ...
            build(): 설정에 대한 정보를 담고있는 Configuration 타입의 객체 혹은 외부 설정 파일과 연결된
                     stream을 매개변수로 전달하면 SqlSesssionFactory 인터페이스 타입의 객체를 반환하는 메소드
         */
        SqlSessionFactory sqlSessionFactory = new SqlSessionFactoryBuilder().build(configuration);
        SqlSession session = sqlSessionFactory.openSession(false);

        Mapper mapper = session.getMapper(Mapper.class); // 쿼리가 드나드는 통로이자 실행하는 역할을 수행
